...
int main(int argc , char** argv )
{
	...
	if (argc == 1 )
	{
		...
		std::cout << "qcbylonlatlut -type geotrans -inf input.tif -reff ref.tif " 
			...
			" [-irx 0.04] [-iry 0.04] " //输入数据分辨率单位度，可选，尽在匹配模式为aver时有用，如果没有这两个值默认使用3x3像元平均
			" [-binwid 0.1] "  //直方图宽度，可选
			<< std::endl ;
		std::cout << std::endl;
	...
	for (int iby = 0; iby < baseYSize; ++iby)
	{
		...
		for (int ibx = 0; ibx < baseXSize; ++ibx)
		{
			...
			if (baseValue >= inputbaseDataValid0 && baseValue <= inputbaseDataValid1 && blonValue>-998 && blatValue>-998 )
			{
				...
				int targetRefX = (int)((blonValue*t[5] - blatValue*t[2] - t[0] * t[5] + t[2] * t[3]) / (t[1] * t[5] - t[2] * t[4]));
				int targetRefY = (int)((blonValue*t[4] - blatValue*t[1] - t[0] * t[4] + t[1] * t[3]) / (t[2] * t[4] - t[1] * t[5]));
				...
				{//找到匹配值
					...
					dataTripleVector.push_back(WTriple(tempInputValue, tempRefValue, bias));
	...
	//输出直方图
	{
		...
		float histStride = (biasMaxValue - biasMinValue ) / histBinCount;//fixed
		std::string binwidstr ;
		if (wft_has_param(argc, argv, "-binwid", binwidstr, false))
		{
			float binwid = atof(binwidstr.c_str());
			histStride = binwid;
			histBinCount = (biasMaxValue - biasMinValue + 1) / histStride;
		}
		...
		double tempBiasMinValue = (int)( biasMinValue / histStride ) * histStride - histStride/2;
		for (size_t it = 0; it < pxCount; ++it)
		{
			//int ihist = (biasVector[it] - biasMinValue) / histStride;
			int ihist = (biasVector[it] - tempBiasMinValue) / histStride;
			if (ihist < 0) ihist = 0; 
			else if (ihist > histBinCount - 1) ihist = histBinCount - 1;
			++ histBinArray[ihist];
			if (it % scaStride == 0) {
		...
		{
			biasdistOfs << tempBiasMinValue + (ih+0.5)* histStride  << " " << histBinArray[ih] / validPixelCount << std::endl;
		}
		delete[] histBinArray;
		biasdistOfs.close();
	...
	{//输出xml
		...
		xmlStream << "<match_method>" << matchMethod << "</match_method>" << std::endl;
