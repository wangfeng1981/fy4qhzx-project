// fy4sncm15monitorforpng.cpp : 定义控制台应用程序的入口点。
//
//该程序不负责检查是否入库和入库操作，只负责生成png
//监控fy4 snc 15min 积雪覆盖产品15min实时数据目录
// 定时调用该程序，扫描目录：
//1.检查该文件是否已经生成png？
//2.0 生成vrt
...
int main(int argc, char** argv)
{
	if (argc == 1)
	{
		cout << "V0.1a Fy4 snow 15min product (L2 SNC NOM) monitor." << endl;
		cout << "by wangfengdev@163.com 2017-10-7." << endl;
		cout << "Sample call:" << endl;
		cout << " fy4sncm15monitorforpng startup.params " << endl;
	...
	string gnuplotProgram = wft_getValueFromExtraParamsFile(startupFile, "#gnuplot", true);
	string gdalwarpProgram = wft_getValueFromExtraParamsFile(startupFile, "#gdalwarp", true);
	...
	const string programid = "fy4sncm15monitorforpng";
	...
	for (size_t i = 0; i < dirFiles.size(); ++i)
	{
		...
		size_t pos1 = tempfile.rfind("L2-_SNC-_MULT_NOM");
	...
	string vrtProgram , string gnuplot , string gdalwarp )
{
	...
	string llvFile = undbFile + ".llv.txt";
	{
		...
		string cmd21 = vrtProgram +
			" -type llfile "
			+ " -in HDF5:\"" + undbFile + "\"://SNC "
			+ " -lonfile " + fy4lonFile
			+ " -latfile " + fy4latFile
			+ " -pdtnodata 0 "
			+ " -pdtlut -1:0,0:0,255:255,256:0 "
			+ " -outvrt " + vrtFile;
		...
		cmd2 += wgs84tif + " -out " + llvFile
			...
			+ " -valid0 0 -valid1 500 -xspace 2 -yspace 2";
