// fy4lstm15monitorforpng.cpp : 定义控制台应用程序的入口点。
//该程序不负责检查是否入库和入库操作，只负责生成png
//监控fy4 lst 15min数据产品，定时调用该程序，扫描目录：
//1.检查该文件是否已经生成png？
//2.0 生成vrt
...
int main(int argc, char** argv)
{
	if (argc == 1)
	{
		cout << "V0.1a Fy4 lst 15min product monitor." << endl;
		cout << "by wangfengdev@163.com 2017-10-6." << endl;
		cout << "Sample call:" << endl;
		cout << " fy4lstm15monitorforpng startup.params " << endl;
	...
	const string programid = "fy4lstm15monitorforpng";
	wft_log(logDir, programid, "begin");

	//FY4A-_AGRI--_N_DISK_1047E_L2-_LST-_MULT_NOM_20170829080000_20170829081459_4000M_V0001.NC
	...
	for (size_t i = 0; i < dirFiles.size(); ++i)
	{
		...
		size_t pos1 = tempfile.rfind("L2-_LST-_MULT_NOM");
	...
	string vrtProgram)
{
	...
	string llvFile = undbFile + ".llv.txt";
	{
		...
		string cmd21 = vrtProgram +
			" -type llfile "
			+ " -in HDF5:\"" + undbFile + "\"://LST "
			+ " -lonfile " + fy4lonFile
			+ " -latfile " + fy4latFile
			+ " -pdtnodata 65535 "
			+ " -pdtlut -1:65536,0:0,500:500,501:65535 "
			+ " -outvrt " + vrtFile;
