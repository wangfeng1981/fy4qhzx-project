// f3mndvionestep.cpp : 定义控制台应用程序的入口点。
//风3b与modis月产品植被指数质检一步计算程序
//wangfeng1@piesat.cn 2017-9-21
...
std::string getValueFromExtraParamsFile(std::string extrafile, std::string key, bool musthas = false)
{
	...
	if (find == false)
	{
		...
		if (musthas)
		{
			std::cout << "Error : Can not found " << key << ", program is out...  " << std::endl;
			exit(111);
		}
	}
	return res;
...
int main(int argc , char** argv )
{
	...
	if (argc != 4)
	{
		std::cout << "Sample call: f3mndvionestep yyyymm[201701] startup.dat output.xml" << std::endl;
		std::cout << "Error: no enough params. out." << std::endl;
		exit(101);
	}

	std::string inputYearMonth = argv[1] ;
	std::string startupFile = argv[2];
